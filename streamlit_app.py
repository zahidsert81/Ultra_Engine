...
# Çerez Dosyası Kontrolü (GitHub depondaki 'cookies.txt' ismine göre sabitlendi)
            ...
            with st.spinner("İşlem yapılıyor..."):
                try:
                    with tempfile.TemporaryDirectory() as tmpdir:
                        # 403 ve Format hatalarını aşmak için en kararlı ayarlar
                        ydl_opts = {
                        ...
                        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                            ...
                            with col2:
                                st.subheader(T["ai"])
                                ai_prompt = f"Summarize this: {info.get('title')} {info.get('description', '')[:300]}"
                    ...
                    if "403" in str(e):
                        st.error("YouTube Erişimi Engellendi! Lütfen GitHub'daki cookies.txt dosyasını yenileyin.")
                    else:
